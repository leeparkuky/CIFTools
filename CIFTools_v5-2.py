...
class ejscreen:  
    def __init__(self, state_fips: Union[ str,  List[str]]):
        self.name = 'https://web.archive.org/web/20250205214353if_/https://gaftp.epa.gov/ejscreen/2024/2.32_August_UseMe/EJScreen_2024_Tract_with_AS_CNMI_GU_VI.csv'
...
class fcc_avail:
    ...
    def download_data(self, state, req):
        ...
        df = df[['FIPS', 'County', 'State', 'pctBB_1000_100', 'pctBB_100_20', 'pct5G_7_1', 'pct5G_35_3']]
...
##################################################################
## CDC SVI (county/tract level SVI)
##################################################################
...
class svi_data:
    ...
    def svi_county(self):
        ...
        df.rename(columns = {"STCNTY" : "FIPS", "COUNTY" : 'County', "STATE" : "State", 
                             "RPL_THEME1" : "SVI_SES", "RPL_THEME2" : "SVI_Household", 
                             "RPL_THEME3" : "SVI_Minority", "RPL_THEME4" : "SVI_Housing",
                             "RPL_THEMES" : "SVI_Overall"}, inplace=True)
...
if __name__ == '__main__':
    ...
    def cdc_svi(state_fips = state_fips):
    ...
    if isinstance(state_fips, str):
        functions = [food_desert_func, ejscreen_func, cdc_risk_and_screening, cdc_svi, bls_func, urban_rural_func, 
                     superfund_func, fcc_func]
        dataset_name = ['food_desert', 'ejscreen', 'cdc', 'svi', 'bls','urban_rural','superfund', 'fcc']
        res = Parallel(n_jobs = -1)(delayed(f)() for f in functions)
        other_data = {k: v for k,v in zip(dataset_name, res)}
    else:
        functions = [food_desert_func, ejscreen_func, cdc_risk_and_screening, cdc_svi, bls_func, 
    ...
    # appending cdc
    sdoh_by_query_level['county']['svi'] = other_data['svi']['county']
    sdoh_by_query_level['tract']['svi']  = other_data['svi']['tract']
