...
def write_excel_file(cdata, full_path, full_path2):
    ...
    with ExcelWriter(full_path, mode = 'w') as writer:
        ...
        cdata['svi_county'].to_excel(writer, sheet_name= 'SVI (County)', index=True)
        cdata['svi_tract'].to_excel(writer, sheet_name= 'SVI (Tract)', index=True)
    ...
    with ExcelWriter(full_path2, mode = 'w') as writer:
        ...
        cdata['rf_and_screening_tract_long'].to_excel(writer, sheet_name= 'RF and Screening (Tract)', 
                                                      index=True)
        cdata['svi_county_long'].to_excel(writer, sheet_name= 'SVI (County)', index=True)
        cdata['svi_tract_long'].to_excel(writer, sheet_name= 'SVI (Tract)', index=True)
...
#     select_area_for_catchment_area = partial(select_area_for_catchment_area_full, ca = ca)
    ...
    #### svi
    svi_county = organize_table_county.select_area_for_catchment_area('svi')
    svi_county.drop(['County'], axis=1, inplace=True)
    svi_countyFix = coFix.merge(svi_county, on = 'FIPS', how = 'left')
    svi_county = svi_countyFix
    svi_tract  = organize_table_tract.select_area_for_catchment_area('svi')
    svi_county_l = pd.melt(svi_county, id_vars=['FIPS', 'County', 'State'], 
                         var_name='measure', value_name='value')
    svi_county_l['value'] = pd.to_numeric(svi_county_l['value'])
    svi_tract_l = pd.melt(svi_tract, id_vars=['FIPS', 'Tract', 'County', 'State'], 
                         var_name='measure', value_name='value')
    svi_tract_l['value'] = pd.to_numeric(svi_tract_l['value']) 
    ...
    cdata = {'rf_and_screening_county': rfs_county, 'rf_and_screening_county_long': rfs_county_l,
                ...
                'svi_county': svi_county, 'svi_county_long': svi_county_l,
                'svi_tract': svi_tract, 'svi_tract_long': svi_tract_l,
