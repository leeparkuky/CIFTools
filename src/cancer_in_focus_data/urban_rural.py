...
def urban_rural_counties(state_fips: Union[str, List[str]]):
    ...
    with NamedTemporaryFile(delete=True, suffix=".xlsx") as temp_file:
        ...
        # Read the dataset
        urban_rural_counties = pd.read_excel(
            temp_filename, dtype={"STATE": str, "COUNTY": str}
        )
    ...
    # Process the data
    urban_rural_counties["FIPS"] = (
        urban_rural_counties.STATE + urban_rural_counties.COUNTY
    )
    urban_rural_counties = urban_rural_counties[
        ["FIPS", "COUNTY_NAME", "STATE_NAME", "POPPCT_URB"]
    ]
    ...
    urban_rural_counties = urban_rural_counties.sort_values("FIPS").reset_index(
        drop=True
    )
