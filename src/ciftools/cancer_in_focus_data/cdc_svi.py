    ...
    def state(self):
        if hasattr(self, "_state"):
            ...
            if isinstance(self.state_fips, str):
                self._state = stateDf.loc[
                    stateDf.FIPS2.eq(self.state_fips), "State"
                ].values[0]
                logger.info(f"State for FIPS {self.state_fips}: {self._state}")
            else:
                self._state = stateDf.loc[
                    stateDf.FIPS2.isin(self.state_fips), "State"
                ].values.tolist()
...
if __name__ == "__main__":
    ...
    print(
        "State: ", svi.state
    )  # Should print the state names corresponding to the FIPS codes
    print(
        "SVI County Data:\n", svi.svi_data["county"].head()
    )  # Print the first few rows of county data
    print(
        "SVI Tract Data:\n", svi.svi_data["tract"].head()
    )  # Print the first few rows of tract data
# %%
