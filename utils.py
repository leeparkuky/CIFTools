...
#         print("%s == %s" % (key, value))
    ...
    with open(bash_file_name, 'w') as f:
        ...
        f.write('clear');
        f.write('\n\n\n')
        f.write(f"python CIFTools_v5-2.py --ca_file_path $ca_file_path --query_level {query_level} --year $year --census_api_key $census_api_key");
        if socrata:
        ...
        if cif_data_pull:
            f.write(f'python CIF_pull_data_v5-2.py --ca_name "$catchment_area_name" --ca_file_path $ca_file_path --pickle_data_path "cif_raw_data.pickle" --download_file_type {download_file_type}')
